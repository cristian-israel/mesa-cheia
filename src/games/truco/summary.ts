...
import { teamTotals } from '@/games/truco/schema'
import { useTrucoStore } from '@/games/truco/store'

export function summarizeTruco(session: Session): SessionSummary {
  const grouped = Boolean(session.teams && session.teams.length > 0)
  const sides = scoringSides(session)
  const state = useTrucoStore.getState().sessions[session.id]
